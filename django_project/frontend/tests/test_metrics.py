...
class SpeciesPopuationCountTestCase(TestCase):
    def setUp(self):
        ...
        self.taxon = TaxonFactory.create(taxon_rank=taxon_rank, common_name_varbatim='Lion')
        ...
        self.property = PropertyFactory.create(organisation=self.organisation_1, name='PropertyA')

        self.owned_species = OwnedSpeciesFactory.create_batch(5, taxon=self.taxon, user=user, property=self.property)

        self.url = reverse('frontend:species_population_count', kwargs={'property_id': self.property.id})
    ...
    def test_species_population_count(self):
        ...
        annual_populations = AnnualPopulation.objects.filter(owned_species=owned_species).values('month').annotate(month_total=Sum('total'))
