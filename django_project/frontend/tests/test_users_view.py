...
class OrganisationUsersViewTest(TestCase):
    ...
    def test_delete_post_method(self):
        ...
        OrganisationUser.objects.filter(user=self.organisation_user.user).delete()

        self.assertEqual(OrganisationUser.objects.filter(user=self.organisation_user.user).exists(), False)
