...
class SpeciesTrend(APIView):
    ...
    def get(self, request):
        ...
        if species_name:
            query &= Q(owned_species__taxon__scientific_name=species_name)

        if property_ids:
            query &= Q(owned_species__property__id__in=property_ids)
        ...
        species = get_object_or_404(Taxon, scientific_name=species_name)

        cached_json_data = get_statistical_model_output_cache(
            species, SPECIE_PER_PROPERTY)
        if cached_json_data:
            return Response(status=200, data=cached_json_data)
        statistical_model_output = StatisticalModelOutput.objects.filter(
            model__taxon=species,
        ...
        remove_plumber_data(data_filepath)
        if is_success:
            national_trend_data = json_data.get(SPECIE_PER_PROPERTY, [])
            save_statistical_model_output_cache(species, SPECIE_PER_PROPERTY,
                                                national_trend_data)
            return Response(status=200, data=national_trend_data)
        return Response(status=200, data=[])
