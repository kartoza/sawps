...
class PropertyCountPerPopulationSizeCategorySerialiser(serializers.ModelSerializer):
    """
    Serializer class for serializing property count per population size category.
    """
    population_category = serializers.SerializerMethodField()

    class Meta:
        model = Taxon
        fields = ["population_category", "graph_icon", "common_name_varbatim", "colour"]

    def get_population_category(self, obj) -> dict:
        """ Calculate property count per population size group.
        Params: obj (Taxon): The Taxon instance.
        """

        filters = {
            'taxon': obj,
            'year': self.context['request'].GET.get('year', timezone.now().year)
        }
        property_list = self.context['request'].GET.get("property")
        if property_list:
            property_ids = property_list.split(",")
        ...
        for idx, category in enumerate(population_categories):
            if idx != len(population_categories) - 1:
                ...
                upper_bound = population_categories[idx + 1] - 1
                ...
                for count in counts:
                    result[count['property__property_type__name']] = count['count']
